<form wire:submit.prevent="submit" id="data-ayah" class="form-area {{ ($status) ? 'orang-tua-nonaktif' : '' }}">
    ...
    <div class="form-row radio">
        <div class="form-col">
            <input id="statusAyah" type="checkbox" wire:model.defer="status">
            <label for="statusAyah">Centang jika Ayah sudah meninggal/tinggal di luar negeri</label>
        </div>
    </div>
    <span class="msg-ortu-nonaktif">Data Tidak perlu diisi</span>

    <label for="">NAMA AYAH</label>
    <input type="text" wire:model.defer="nama" required minlength="3" maxlength="60" placeholder="Nama Ayah">
    ...
    <input type="text" wire:model.defer="alamat" required minlength="3" maxlength="255" placeholder="Alamat Ayah">
    @error('alamat') 
    ...
    <script>
        $(document).ready(function() {
            $('#statusAyah').change(function() {
                if ($(this).is(':checked')) {
                    $('#data-ayah').addClass('orang-tua-nonaktif');
                } else {
                    $('#data-ayah').removeClass('orang-tua-nonaktif');
                }
            });
        });
    </script>
