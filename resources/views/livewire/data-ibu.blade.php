<form wire:submit.prevent="submit" id="data-ibu" class="form-area {{ ($status) ? 'orang-tua-nonaktif' : '' }}">
    ...
    <div class="form-row radio">
        <div class="form-col">
            <input id="statusIbu" type="checkbox" wire:model.defer="status">
            <label for="statusIbu">Centang jika Ibu sudah meninggal/tinggal di luar negeri</label>
        </div>
    </div>
    <span class="msg-ortu-nonaktif">Data Tidak perlu diisi</span>

    <label for="">NAMA IBU</label>
    <input type="text" wire:model.defer="nama" required minlength="3" maxlength="60" placeholder="Nama Ibu">
    ...
    <script>
        $(document).ready(function() {
            $('#statusIbu').change(function() {
                if ($(this).is(':checked')) {
                    $('#data-ibu').addClass('orang-tua-nonaktif');
                } else {
                    $('#data-ibu').removeClass('orang-tua-nonaktif');
                }
            });
        });
    </script>
